...
if not exist "%EXE%" (
  ...
  echo Pozri README-RUST.txt, kapitolu o antiviruse.
  pause
  exit /b 1
)
...
if "%PRIECINOK%"=="" (
  ...
  echo Priklad: C:\Users\TvojeMeno\Desktop\Zvuky
  set /p PRIECINOK="> "
)
if "%PRIECINOK%"=="" echo Nezadana cesta - koniec. & pause & exit /b 1
if not exist "%PRIECINOK%" (
  echo Priecinok neexistuje: %PRIECINOK%
  pause
  exit /b 1
)
...
echo Tip: priecinok so zvukmi mozes aj potiahnut myskou na TEST-RUST.bat
echo.
"%EXE%" "%PRIECINOK%" --popisy "%POPISY%" %MODELDIR%
echo.
echo Hotovo - stlac lubovolny klaves na zatvorenie.
pause >nul
